...
export class ProfitMaxiClient {
   ...
   *
   * The IDL is loaded from `../idl/profitmaxi.json` (generated by `anchor build`).
   * Run `anchor build` and copy `target/idl/profitmaxi.json` to `sdk/src/idl/` before
   * using any program methods.
   */
  private getProgram(): Program {
    if (!this.program) {
      ...
      try {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        idl = require('./idl/profitmaxi.json') as Idl;
      } catch {
        throw new Error(
          'ProfitMaxi IDL not found. Run `anchor build` and copy ' +
          '`target/idl/profitmaxi.json` to `sdk/src/idl/profitmaxi.json`.'
        );
      }
